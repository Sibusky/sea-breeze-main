...
// Навигатор для скроллинга
const navigation = document.querySelector('.outfit-header__navigation-list');
...
// Скролл до нужной статьи
function scrollToArticle(e) {
  // Сначала ищу ID линка, потом делаю скролл до статьи с тем же ID
  const linkId = e.target.closest('li').id.split('-').pop();
  document.getElementById(`article-${linkId}`).scrollIntoView({
    behavior: 'smooth',
  });
}
...
// Навигатор для скроллинга
navigation.addEventListener('click', scrollToArticle);
